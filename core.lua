--!strict
if getgenv().FractureLoaded then
  return
end
getgenv().FractureLoaded = true
...
repeat
  Luna = loadstring(game:HttpGet("https://raw.githubusercontent.com/xinjuri/k/refs/heads/main/Luna.lua"))
  ...
  if not Luna then
    warn("[Fracture]: Failed to load Luna, retrying")
  end
...
local Window = Luna:CreateWindow({
  Name = `Fracture | {PlaceName}`,
  Subtitle = `{ScriptVersion or "Dev Mode"}`,
  LogoID = "82795327169782",
  LoadingEnabled = true,
  LoadingTitle = `Fracture's {ScriptVersion or "Dev Mode"}`,
  ...
  HideIdentity = {
    ...
    },
    {
      ...
      Info = {
        ...
        Callback = function()
          Notify("Fracture", "UI Destroyed", "user-round-check", "Lucide")
          getgenv().FractureLoaded = nil
          Luna:Destroy()
        end
      }
    }
...
getgenv().CreateUniversalTabs = function()
  ...
  Notify("Welcome to Fracture", `Loaded in {math.floor((tick() - StartLoadTime) * 10) / 10}s`, "loader", "Lucide")
end
